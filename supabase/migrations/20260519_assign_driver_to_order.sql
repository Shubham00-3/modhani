-- Driver assignment per order.
--
-- Today the driver portal shows every shipped order to every driver. We need:
--   1. Admin can assign a specific driver to each order.
--   2. Driver portal shows ONLY orders assigned to that driver (and only
--      while they're still in "shipped" state - once they POD it, the order
--      flips to "delivered" and disappears from the queue).
--   3. Every assignment / reassignment / clear must write an audit row.
...
alter table public.orders
  ...
  add column if not exists driver_assigned_by uuid references auth.users (id) on delete set null;

create index if not exists orders_driver_user_id_idx
  on public.orders (driver_user_id) where driver_user_id is not null;

-- ---------------------------------------------------------------------------
-- Assign / reassign / clear driver
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Resolve the previous driver's name for the audit row.
