-- Per-line invoice discount.
--
-- Adds a flat-dollar discount that subtracts from the line total
-- (qty × unit price − discount_amount, floored at 0). Invoice total is
-- the sum of those discounted line totals.

alter table public.order_items
  add column if not exists discount_amount numeric(12, 2) not null default 0;

-- ---------------------------------------------------------------------------
-- Updated create-invoice RPC: accepts "discount" per override row.
-- ---------------------------------------------------------------------------
drop function if exists public.modhanios_create_invoice(text, uuid, text, jsonb, timestamptz, text, text, text, text, text, text, text);
...
begin
  ...
  if exists (
    with overrides as (
      ...
      from jsonb_to_recordset(p_overrides) as x("orderItemId" text, "overridePrice" numeric, "overrideReason" text, "discount" numeric)
    )
    select 1
    from overrides o
...
-- ---------------------------------------------------------------------------
-- Updated edit-invoice RPC: accepts "discount" per line.
-- ---------------------------------------------------------------------------
